...
One thing stops a descriptor, and it names itself in the refusal:
...
A map whose key needs a generated hash used to be a second stop and is not one
now. What closed it is below, under "the driver resolves a generated name
...
where `gc` decides it too, and it finds the equality and hash functions the
same way: a descriptor names the symbol and whoever writes the descriptor
defines it.

#### The driver resolves a generated name against the closed set

`driver/compile.go` scans the symbols `rtype` returned for every type in the
closed set and generates the body of each `type:.eq.` or `type:.hash.` name it
finds. What it may not do is assume that the name belongs to the type whose
descriptor holds it. A struct's `Equal` names its own equality function, and a
map's `Hasher` names the **key's** hash, so for one of the two the descriptor
that owes the function and the type that owns it are different types.

So the driver builds one index over the whole closed set, from each type's two
generated symbols to that type, and resolves every name it meets through it.
`Referenced` puts a map's key in the closure, so the owner of every generated
name a descriptor in the set can point at is in the set as well. A name in the
family that the index does not hold is reported rather than skipped: skipping
it is a relocation `cmd/link` resolves against nothing, and the diagnostic
would then name a symbol instead of a type.

`mapEmittable` asks `hashClosure` for the key and not `hashFunc`, which is the
same rule this spec applies everywhere else: the check has to ask the writer.
`hashFunc` answers only the hashes the runtime carries, so asking it refused
every map whose key needs a generated hash while the writer beside it was
ready to name one.
...
1. **`driver/compile.go` now calls `ir.Lower`.** It is the first stage of
   ...
   what it would buy: 41,500 of 41,802 distribution functions get past
   construction with the pass, and 21,380 without it.
...
### A map's key hash is generated by whoever writes the map's descriptor

The refusal table above used to carry a second stop: a map whose key needs a
generated hash. Both halves of the mechanism existed. `ssagen.HashFunc` wrote
the body and `rtype`'s `generatedClosure` named it from the map's `Hasher`.
What was missing was between them: `driver/compile.go` resolved a generated
name against the type whose descriptor named it, and the only descriptor that
names a key's hash is the **map's**, so the name matched nothing and the body
was never written. `mapEmittable` refused the map rather than emit a
relocation against `type:.hash.K` with nothing defining it, which was the
right call and not the fix.

The fix is one index over the closed descriptor set, described above. It costs
two symbol names per type and it removes the only place where the driver made a
second decision about which type a name belongs to.
