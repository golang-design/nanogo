...
| --- | --- | --- |
| arm64 instruction encodings | 963,460, counted by the package itself | `go tool asm` |
...
| --- | --- | --- |
| GC stack maps | Allocation stress with `GOGC=1`, plus `GODEBUG=gccheckmark=1` which makes the collector verify its own marking. The [`stackmap` spike](../spikes/stackmap) is the shape of the test. | built, `ssagen/gc_test.go`, under `gccheckmark=1,clobberfree=1` and `GOGC=1`; the corpus maps 8,237 of the 8,238 functions it lowers |
