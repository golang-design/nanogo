...
**The table is built and gated.** `rtsym` holds 45 symbols and its test checks
every one against 2,435 functions parsed out of the runtime's source: 0
missing, 0 differing. One of the 45 is defined in assembly and is checked by a
different test, because no Go source states its signature.

**Fifteen of the 45 are generated.** Every other row of the tables below names
a call nothing produces yet:
