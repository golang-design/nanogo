...
| [013](013-generics.md) | Generics | `draft` | full stenciling; built for a generic the package declares, refused for a generic type and for another package's |
...
| [034](034-write-barriers.md) | Write barriers | `draft` | `ssa/writebarrier.go`; the three directives are parsed and unread |
...
| [047](047-abi-wrappers.md) | ABI wrappers | `in progress` | G3; stages 0, 1 and 2 built: the runtime gate, the symabis reader, an `-asmhdr` that matches `gc` byte for byte, the `//go:linkname` name model, the ABIInternal wrapper a Go call to an ABI0 assembly definition needs, and `args_stackmap` and `arginfo0` that match `gc` byte for byte. Stage 3's ABI0 wrapper and the rename half of `//go:linkname` are unbuilt |
...
| Not started | Escape analysis, inlining, generics instantiation, itabs, the conversion to an interface that a `panic` of anything else and a read of `recover`'s value both need, the linker, the assembler, DWARF, and the amd64 backend |
