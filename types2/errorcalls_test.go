...
// Source: ../upstream/types2/errorcalls_test.go.txt
	...
	for _, file := range files {
		syntax.Inspect(file, func(n syntax.Node) bool {
			...
			if n := len(call.ArgList); n < errorfMinArgCount {
				t.Errorf("%s: got %d arguments, want at least %d", position(call.Pos()), n, errorfMinArgCount)
				return false
			}
				...
				if lit, _ := n.(*syntax.BasicLit); lit != nil && lit.Kind == syntax.StringLit {
					if s, err := strconv.Unquote(lit.Value); err == nil {
						if !balancedParentheses(s) {
							t.Errorf("%s: unbalanced parentheses/brackets", position(lit.Pos()))
						}
					}
					return false
				}
