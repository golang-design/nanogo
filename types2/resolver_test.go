...
// Source: ../upstream/types2/resolver_test.go.txt
...
func TestResolveIdents(t *testing.T) {
		...
		syntax.Inspect(f, func(n syntax.Node) bool {
			if s, ok := n.(*syntax.SelectorExpr); ok {
				if x, ok := s.X.(*syntax.Name); ok {
					obj := uses[x]
					if obj == nil {
						t.Errorf("%s: unresolved qualified identifier %s", position(x.Pos()), x.Value)
						return false
					}
					if _, ok := obj.(*PkgName); ok && uses[s.Sel] == nil {
						t.Errorf("%s: unresolved selector %s", position(s.Sel.Pos()), s.Sel.Value)
						return false
					}
					return false
				}
	...
	for id, obj := range uses {
		if obj == nil {
			t.Errorf("%s: Uses[%s] == nil", position(id.Pos()), id.Value)
		}
	}
	...
	for _, f := range files {
		syntax.Inspect(f, func(n syntax.Node) bool {
			if x, ok := n.(*syntax.Name); ok {
				...
				switch objects {
				case 0:
					t.Errorf("%s: unresolved identifier %s", position(x.Pos()), x.Value)
				case 3:
	...
	for x := range uses {
		if !foundUses[x] {
			t.Errorf("%s: identifier %s not present in source", position(x.Pos()), x.Value)
		}
	}
	for x := range defs {
		if !foundDefs[x] {
