...
// Source: ../upstream/types2/issues_test.go.txt
...
`
	...
	for x, tv := range types {
		if _, ok := x.(*syntax.CallExpr); ok {
			if tv.Type != want {
				t.Errorf("%s: got %s; want %s", position(x.Pos()), tv.Type, want)
			}
			n++
		}
	}
...
L7 uses var z int`
	...
	for id, obj := range defs {
		if obj != nil {
			fact := fmt.Sprintf("L%d defs %s", position(id.Pos()).Line, obj)
			facts = append(facts, fact)
		}
	}
	for id, obj := range uses {
		fact := fmt.Sprintf("L%d uses %s", position(id.Pos()).Line, obj)
		facts = append(facts, fact)
	}
	slices.Sort(facts)
...
func TestIssue43124(t *testing.T) {
	...
	mustHaveGoBuild(t) // The go command is needed for the importer to determine the locations of stdlib .a files.
...
.*want M5[(]struct{b[.]S; t}[)]`},
	...
	test := func(main, b, want string) {
		...
		mast := mustParse(main)
		conf := Config{Fset: testFset, Importer: importHelper{pkg: bpkg}}
