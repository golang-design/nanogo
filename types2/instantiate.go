...
// Source: ../upstream/types2/instantiate.go.txt
...
func Instantiate(ctxt *Context, orig Type, targs []Type, validate bool) (Type, error) {
	...
	if !ok {
		panic(sprintf(nil, nil, false, "cannot instantiate non-generic %s: expected *Named, *Alias, or *Signature", orig))
	}
	if len(targs) == 0 {
		panic(sprintf(nil, nil, false, "cannot instantiate %s: empty type argument list", orig))
	}
...
func (check *Checker) instance(pos syntax.Pos, orig genericType, targs []Type, expanding *Named, ctxt *Context) (res Type) {
	...
	default:
		...
		panic(fmt.Sprintf("%v: cannot instantiate %v", check.position(pos), orig))
	}
...
func (check *Checker) validateTArgLen(pos syntax.Pos, name string, want, got int) bool {
	...
	panic(fmt.Sprintf("%v: %s", check.position(pos), msg))
}
