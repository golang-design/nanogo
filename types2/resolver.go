...
// Source: ../upstream/types2/resolver.go.txt
...
func (check *Checker) arity(pos syntax.Pos, names []*syntax.Name, inits []syntax.Expr, constDecl, inherited bool) {
	...
	case l < r:
		n := inits[l]
		if inherited {
			check.errorf(pos, code, "extra init expr at %s", check.position(n.Pos()))
		} else {
...
func (check *Checker) filename(fileNo int) string {
	...
	if pos := file.Pos(); pos.IsKnown() {
		return check.position(pos).Filename
	}
...
func (check *Checker) collectObjects() {
	...
	for fileNo, file := range check.files {
		check.version = asGoVersion(check.versions[check.srcFile(file.Pos())])
		...
		fileDir := dir(check.position(file.PkgName.Pos()).Filename)
