...
// Source: ../upstream/types2/check.go.txt
...
import (
	...
	. "golang.design/x/nanogo/types2/errors"
	"os"
)
...
type Checker struct {
	...
	*Info
	...
	versions      map[*syntax.SrcFile]string // maps files to version strings (each file has an entry); shared with Info.FileVersions if present; may be unaltered Config.GoVersion
...
func (check *Checker) initFiles(files []*syntax.File) {
	...
	if versions == nil {
		versions = make(map[*syntax.SrcFile]string)
	}
	...
	for _, file := range check.files {
		...
		// nanogo's syntax.File has no GoVersion field. A //go:build line that
...
func (check *Checker) processDelayed(top int) {
	...
	for i := top; i < len(check.delayed); i++ {
		...
		if check.conf.Trace {
			if a.desc != nil {
				check.trace(atPos(a.desc.pos), "-- "+a.desc.format, a.desc.args...)
			} else {
